...
# 需要计算对数收益率的价格型变量
PRICE_TICKERS = ["美元指数", "美国汽油价格"]

start = "2026-02-28"
end   = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%d")

print(f"Fetching {start} ~ {end} for: {list(TICKERS.keys())} ...")
...
for idx, (date, row) in enumerate(zip(all_dates, all_rows)):
    ...
    if not raw:
        print(f"  {date.date()}: no data, skipping")
        continue
    ...
    if existing:
        ...
        if len(existing_raw) > original_len:
            ...
            skipped += 1
            print(f"  {date.date()}: already has all indicators, skipping")
        continue
